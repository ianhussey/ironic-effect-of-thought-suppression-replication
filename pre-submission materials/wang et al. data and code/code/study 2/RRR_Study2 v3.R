...
df1 <- data.frame(Lab = NA_character_, Researcher = NA_character_, Country = NA_character_, Citation = NA_character_,
                  ...
                  N.conc.initial = NA_real_, N.supp.initial = NA_real_,
                  N.gorilla.initial = 0, N.tiger.initial = 0, N.food.initial = 0, N.diseases.initial = 0,
                  ...
                  t.mean.conc2 = NA_real_, #transformed means, all words, period 2
                  t.mean.supp2 = NA_real_,
                  t.sd.conc2 = NA_real_,
                  t.sd.supp2 = NA_real_,
                  
                  mean.conc1.tiger = NA_real_, #raw means, tiger words, period 1
                  mean.supp1.tiger = NA_real_,
                  sd.conc1.tiger = NA_real_,
                  sd.supp1.tiger = NA_real_,
                  
                  mean.conc2.tiger = NA_real_, #raw means, tiger words, period 2
                  ...
                  mean.conc1.gorilla = NA_real_, #raw means, gorilla words, period 1
                  mean.supp1.gorilla = NA_real_,
                  sd.conc1.gorilla = NA_real_,
                  sd.supp1.gorilla = NA_real_,
                  
                  mean.conc2.gorilla = NA_real_, #raw means, gorilla words, period 2
                  ...
                  mean.conc1.food = NA_real_, #raw means, food words, period 1
                  mean.supp1.food = NA_real_,
                  sd.conc1.food = NA_real_,
                  sd.supp1.food = NA_real_,
                  
                  mean.conc2.food = NA_real_, #raw means, food words, period 2
                  ...
                  mean.conc1.diseases = NA_real_, #raw means, diseases words, period 1
                  mean.supp1.diseases = NA_real_,
                  sd.conc1.diseases = NA_real_,
                  sd.supp1.diseases = NA_real_,
                  
                  mean.conc2.diseases = NA_real_, #raw means, diseases words, period 2
                  ...
                  mean.conc2.male = NA_real_, #raw means, for males, period 2
                  ...
                  mean.conc2.female = NA_real_, #raw means, for females, period 2
                  ...
                  d3.yi.p2 = NA_real_,            #all words, raw means, period 2
                  d3.vi.p2 = NA_real_,
                  d3.yi.transf.p2 = NA_real_,     #all words, transformed, period 2
                  d3.vi.transf.p2 = NA_real_,
                  
                  d3.yi.tiger.p2 = NA_real_,      #tiger, raw means, period 2
                  d3.vi.tiger.p2 = NA_real_,
                  d3.yi.gorilla.p2 = NA_real_,    #gorilla, raw means, period 2
                  d3.vi.gorilla.p2 = NA_real_,
                  d3.yi.food.p2 = NA_real_,       #food, raw means, period 2
                  d3.vi.food.p2 = NA_real_,
                  d3.yi.diseases.p2 = NA_real_,   #diseases, raw means, period 2
                  d3.vi.diseases.p2 = NA_real_,
                  
                  d3.yi.male = NA_real_,       #males, raw means, period 2
                  d3.vi.male = NA_real_,
                  d3.yi.female = NA_real_,     #females, raw means, period 2
                  d3.vi.female = NA_real_,
                  
                  d3.yi.p1 = NA_real_,            #all words, raw means, period 1
                  d3.vi.p1 = NA_real_,
                  d3.yi.transf.p1 = NA_real_,     #all words, transformed, period 1
                  d3.vi.transf.p1 = NA_real_,
                  
                  d3.yi.tiger.p1 = NA_real_,      #tiger, raw means, period 1
                  d3.vi.tiger.p1 = NA_real_,
                  d3.yi.gorilla.p1 = NA_real_,    #gorilla, raw means, period 1
                  d3.vi.gorilla.p1 = NA_real_,
                  d3.yi.food.p1 = NA_real_,       #food, raw means, period 1
                  d3.vi.food.p1 = NA_real_,
                  d3.yi.diseases.p1 = NA_real_,   #diseases, raw means, period 1
                  d3.vi.diseases.p1 = NA_real_,
...
for (i in 1:37) { #Although it is more elegant to use seq_along(), using 1:n is better for debugging. Max n = 37.
  if (i %in% c(1, 4)) next #exclude the first lab because it has too few participants (n = 13)
  start_time <- Sys.time()
...
df1$pct.Probe.pass <-sum(RRR_Wegner87$probe.final == "pass", na.rm = TRUE)/(sum(RRR_Wegner87$probe.final== "pass", na.rm = TRUE)+sum(RRR_Wegner87$probe.final== "fail", na.rm = TRUE))*100
  ...
  #Record counts by conditions (before subsetting)
df1$N.conc.initial = nrow(filter(RRR_Wegner87, Cond == "Concentration")) 
df1$N.supp.initial = nrow(filter(RRR_Wegner87, Cond == "Suppression"))
df1$N.gorilla.initial = nrow(filter(RRR_Wegner87, Targetword == "gorilla"))
df1$N.tiger.initial = nrow(filter(RRR_Wegner87, Targetword == "tiger"))
df1$N.food.initial = nrow(filter(RRR_Wegner87, Targetword == "food"))
df1$N.diseases.initial = nrow(filter(RRR_Wegner87, Targetword == "diseases"))
...
#Transform dependent variable (used in Wegner et al., 1987) - not preregistered
...
RRR_Wegner87$t.intrusions2 <- sqrt(RRR_Wegner87$intrusions2) #indeed the sqrt-transformed intrusion results in a more normal distribution
  #Afterthought: It's actually pointless to do the transformation because some distributions violate normality, some don't.
  #In a meta-analysis, either all transformed DVs are used, or all non-transformed DVs are used. It is wrong to selectively choose which DV to use for each study.
...
#Record counts by conditions (after subsetting)
...
df1$t.sd.conc2<- sd(RRR_Wegner87$t.intrusions2[RRR_Wegner87$Cond == 'Concentration'], na.rm = TRUE)
  ...
  #Conditional means (raw means) by target words
    #Period 1 (note that supp cond for Period 1 is irrelevant and hence not in the codes)
df1$mean.conc1.tiger = df4[df4$Cond == "Concentration" & df4$Targetword == "tiger", "Mean_Targetword"][[1]]
df1$sd.conc1.tiger = df4[df4$Cond == "Concentration" & df4$Targetword == "tiger", "SD_Targetword"][[1]]

df1$mean.conc1.gorilla = df4[df4$Cond == "Concentration" & df4$Targetword == "gorilla", "Mean_Targetword"][[1]]
df1$sd.conc1.gorilla = df4[df4$Cond == "Concentration" & df4$Targetword == "gorilla", "SD_Targetword"][[1]]

df1$mean.conc1.food = df4[df4$Cond == "Concentration" & df4$Targetword == "food", "Mean_Targetword"][[1]]
df1$sd.conc1.food = df4[df4$Cond == "Concentration" & df4$Targetword == "food", "SD_Targetword"][[1]]

df1$mean.conc1.diseases = df4[df4$Cond == "Concentration" & df4$Targetword == "diseases", "Mean_Targetword"][[1]]
df1$sd.conc1.diseases = df4[df4$Cond == "Concentration" & df4$Targetword == "diseases", "SD_Targetword"][[1]]
...
####Effect size calculation for meta-analysis
  #Raw means (all words), period 2
d3.p2 <- escalc(n2i = N.conc, n1i = N.supp, 
...
df1$d3.yi.p2 <- d3.p2$yi #paste Cohen's d to df1
df1$d3.vi.p2 <- d3.p2$vi #variance of Cohen's d

  #Raw means (all words), period 1 conc vs. period 2 conc after supp. 
    #Note: The suffix for the variables end with .p1, but this does NOT mean the codes are comparing the two conditions in period 1.
d3.p1 <- escalc(n2i = N.conc, n1i = N.supp, #Note: Sample sizes can be kept as the original conc vs. supp
                m2i = mean.conc2, m1i = mean.conc1, #Note: m1i is Period 1's concentration condition
                sd2i = mean.conc2, sd1i = mean.conc1, data = df1, measure = "SMD", 
                append = TRUE) #Let op!  The effect size calculation is M1-M2, not M2-M1.
df1$d3.yi.p1 <- d3.p1$yi
df1$d3.vi.p1 <- d3.p1$vi

  #Transformed means (all words), period 2 (Period 1 comparison not relevant, hence not in codes below)
d3.transf.p2 <- escalc(n2i = N.conc, n1i = N.supp, 
             m2i = t.mean.conc2, m1i = t.mean.supp2, 
             sd2i = t.mean.conc2, sd1i = t.mean.supp2, data = df1, measure = "SMD", 
             append = TRUE)
df1$d3.yi.transf.p2 <- d3.transf.p2$yi
df1$d3.vi.transf.p2 <- d3.transf.p2$vi

  #Raw means (by target words), period 2
d3.tiger.p2 <- escalc(n2i = n.conc.tiger, n1i = n.supp.tiger, 
             m2i = mean.conc2.tiger, m1i = mean.supp2.tiger, 
             sd2i = mean.conc2.tiger, sd1i = mean.supp2.tiger, data = df1, measure = "SMD", 
             append = TRUE)
df1$d3.yi.tiger.p2 <- d3.tiger.p2$yi
df1$d3.vi.tiger.p2 <- d3.tiger.p2$vi

d3.gorilla.p2 <- escalc(n2i = n.conc.gorilla, n1i = n.supp.gorilla, 
                   m2i = mean.conc2.gorilla, m1i = mean.supp2.gorilla, 
                   sd2i = mean.conc2.gorilla, sd1i = mean.supp2.gorilla, data = df1, measure = "SMD", 
                   append = TRUE)
df1$d3.yi.gorilla.p2 <- d3.gorilla.p2$yi
df1$d3.vi.gorilla.p2 <- d3.gorilla.p2$vi

d3.food.p2 <- escalc(n2i = n.conc.food, n1i = n.supp.food, 
                   m2i = mean.conc2.food, m1i = mean.supp2.food, 
                   sd2i = mean.conc2.food, sd1i = mean.supp2.food, data = df1, measure = "SMD", 
                   append = TRUE)
df1$d3.yi.food.p2 <- d3.food.p2$yi
df1$d3.vi.food.p2 <- d3.food.p2$vi

d3.diseases.p2 <- escalc(n2i = n.conc.diseases, n1i = n.supp.diseases, 
                   m2i = mean.conc2.diseases, m1i = mean.supp2.diseases, 
                   sd2i = mean.conc2.diseases, sd1i = mean.supp2.diseases, data = df1, measure = "SMD", 
                   append = TRUE)
df1$d3.yi.diseases.p2 <- d3.diseases.p2$yi
df1$d3.vi.diseases.p2 <- d3.diseases.p2$vi
...
df1$d3.vi.female <- d3.female$vi
  ...
  #Raw means (by target words), period 1
    #Note: The suffix for the variables end with .p1, but this does NOT mean the codes are comparing the two conditions in period 1.
d3.tiger.p1 <- escalc(n2i = n.conc.tiger, n1i = n.supp.tiger, #Note: Sample sizes can be kept as the original conc vs. supp
                      m2i = mean.conc2.tiger, m1i = mean.conc1.tiger,  #Note: m1i is Period 1's concentration condition
                      sd2i = mean.conc2.tiger, sd1i = mean.conc1.tiger, data = df1, measure = "SMD", 
                      append = TRUE)
df1$d3.yi.tiger.p1 <- d3.tiger.p1$yi
df1$d3.vi.tiger.p1 <- d3.tiger.p1$vi

d3.gorilla.p1 <- escalc(n2i = n.conc.gorilla, n1i = n.supp.gorilla, #Note: Sample sizes can be kept as the original conc vs. supp
                        m2i = mean.conc2.gorilla, m1i = mean.conc1.gorilla, #Note: m1i is Period 1's concentration condition
                        sd2i = mean.conc2.gorilla, sd1i = mean.conc1.gorilla, data = df1, measure = "SMD", 
                        append = TRUE)
df1$d3.yi.gorilla.p1 <- d3.gorilla.p1$yi
df1$d3.vi.gorilla.p1 <- d3.gorilla.p1$vi

d3.food.p1 <- escalc(n2i = n.conc.food, n1i = n.supp.food, #Note: Sample sizes can be kept as the original conc vs. supp
                     m2i = mean.conc2.food, m1i = mean.conc1.food, #Note: m1i is Period 1's concentration condition
                     sd2i = mean.conc2.food, sd1i = mean.conc1.food, data = df1, measure = "SMD", 
                     append = TRUE)
df1$d3.yi.food.p1 <- d3.food.p1$yi
df1$d3.vi.food.p1 <- d3.food.p1$vi

d3.diseases.p1 <- escalc(n2i = n.conc.diseases, n1i = n.supp.diseases, #Note: Sample sizes can be kept as the original conc vs. supp
                         m2i = mean.conc2.diseases, m1i = mean.conc1.diseases, #Note: m1i is Period 1's concentration condition
                         sd2i = mean.conc2.diseases, sd1i = mean.conc1.diseases, data = df1, measure = "SMD", 
                         append = TRUE)
df1$d3.yi.diseases.p1 <- d3.diseases.p1$yi
df1$d3.vi.diseases.p1 <- d3.diseases.p1$vi
...
Study2.ms.df1 <- data.frame (Citation = df1$Citation,
                             Country = df1$Country,
...
write.csv(Study2.ms.df1, "C:/Users/jc506932/Dropbox/RRR (Adam Wang)/RRR_Wegner87_table2.csv") #Not useful for pasting because of ü characters in two names.
...
mean(df1$mean.age, na.rm = T) #20.38
sd(df1$mean.age, na.rm = T) #1.42
...
mean(df1$ICC.ABC.mean) #92.97%
mean(df1$IRR.probe) #98.70%
...
count_lower <- print(sum(df1$p.Cond.Targetword.raw < .05)) #Number of labs with significant 2 (initial instructions: suppression vs. concentration) × 4 (target thought: gorilla vs. tiger vs. food vs. diseases) ANOVA 
...
#d3: Rebound effect (raw means)
  #Period 2: Conc vs Supp
overalld3.p2 <- rma(yi = d3.yi.p2, vi = d3.vi.p2, data = df1)
summary(overalld3.p2)

  #Period 1 Conc vs Period 2 Conc
overalld3.p1 <- rma(yi = d3.yi.p1, vi = d3.vi.p1, data = df1)
summary(overalld3.p1)

#d3: Rebound effect (transformed means) - Not preregistered, period 2
overalld3.transf.p2 <- rma(yi = d3.yi.transf.p2, vi = d3.vi.transf.p2, data = df1)
summary(overalld3.transf.p2)

#d3: Rebound effect by target words (preregistered, directional)
  #Period 2: Conc vs Supp
overalld3.tiger.p2 <- rma(yi = d3.yi.tiger.p2, vi = d3.vi.tiger.p2, data = df1)
summary(overalld3.tiger.p2)

overalld3.gorilla.p2 <- rma(yi = d3.yi.gorilla.p2, vi = d3.vi.gorilla.p2, data = df1)
summary(overalld3.gorilla.p2)

overalld3.food.p2 <- rma(yi = d3.yi.food.p2, vi = d3.vi.food.p2, data = df1)
summary(overalld3.food.p2)

overalld3.diseases.p2 <- rma(yi = d3.yi.diseases.p2, vi = d3.vi.diseases.p2, data = df1)
summary(overalld3.diseases.p2)
  
  #Period 1 Conc vs Period 2 Conc
overalld3.tiger.p1 <- rma(yi = d3.yi.tiger.p1, vi = d3.vi.tiger.p1, data = df1)
summary(overalld3.tiger.p1)

overalld3.gorilla.p1 <- rma(yi = d3.yi.gorilla.p1, vi = d3.vi.gorilla.p1, data = df1)
summary(overalld3.gorilla.p1)

overalld3.food.p1 <- rma(yi = d3.yi.food.p1, vi = d3.vi.food.p1, data = df1)
summary(overalld3.food.p1)

overalld3.diseases.p1 <- rma(yi = d3.yi.diseases.p1, vi = d3.vi.diseases.p1, data = df1)
summary(overalld3.diseases.p1)

#d3: Rebound effect by gender (pre-registered as exploratory, nondirectional), period 2
...
#d3: Rebound effect with Region as a moderator, period 2
overalld3_region <- rma(yi = d3.yi.p2, vi = d3.vi.p2, mods = ~ Region, data = df1)
...
print(asian_pred)
  ...
  #Simple summary (unweighted effect size!) for quick interpretation of the direction
df1 %>%
  group_by(Region) %>%
  summarise(MeanValue = mean(d3.yi.p2, na.rm = TRUE))

#d3: Rebound effect with Self-reported thought occurrence percentages as a moderator
overalld3_pctthinking <- rma(yi = d3.yi.p2, vi = d3.vi.p2, mods = ~ Pctthinking.phase.2, data = df1)
summary(overalld3_pctthinking)

# Prepare the data for the forest plot
base_data <- tibble::tibble(mean  = df1$d3.yi.p2,
                            lower = df1$d3.yi.p2-1.96*sqrt(df1$d3.vi.p2),
                            upper = df1$d3.yi.p2+1.96*sqrt(df1$d3.vi.p2),
                            Lab = df1$Citation,
                            Country = df1$Country,
                            ES = sprintf("%.2f", df1$d3.yi.p2)) #2 decimal places

box_sizes <- 1/sqrt(df1$d3.vi.p2) 
box_sizes <- box_sizes / max(box_sizes) * 0.3  # scale to max = 0.3

png("C:/Users/jc506932/Dropbox/RRR (Adam Wang)/Forestplot2.png", width = 21, height = 29.7, units = "cm", res = 600)
base_data |>
  forestplot(labeltext = c(Lab, ES, Country),
             clip = c(-3, 3),
             boxsize = box_sizes,
             align = c("l", "l", "l"), #alignment of text
             txt_gp = fpTxtGp(
             ...
             ), lineheight = unit(0.7, "cm"), xlab = "Cohen's d",
             )|>
  ...
  fp_set_style(box = "royalblue",
               line = "darkblue",
               summary = "red") |>
  fp_add_header(Lab = c("", "Author(s)"),
                Country = c("", "Country"),
                ES = c("", "Cohen's d")) |>
  fp_append_row(mean  = overalld3.p2$beta[1],
                lower = overalld3.p2$ci.lb,
                upper = overalld3.p2$ci.ub,
                Lab = "Summary",
                Country = "",
                ES = "0.12",
                is.summary = TRUE) |>
